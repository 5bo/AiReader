...
/**
 * 作者    武博
 * 时间    2016/6/12 0012 15:27
 * 文件    NetUtils.java
 * 描述    网络工具
 * {@link #getNetworkType(Context)}  获取网络类型
 * {@link #getNetworkTypeName(Context)}  获取网络名称
 * {@link #isConnected(Context)}  检查网络状态
 * {@link #isNetworkAvailable(Context)}  网络可用性
 * {@link #isWiFi(Context)}  是否wifi
 * {@link #openNetSetting(Activity)}  打开网络设置界面
 * {@link #setWifiEnabled(Context, boolean)}  设置wifi状态
 * {@link #getWifiScanResults(Context)}  获取wifi列表
 * {@link #getScanResultsByBSSID(Context, String)}  过滤扫描结果
 * {@link #getWifiConnectionInfo(Context)}  获取wifi连接信息
 */
